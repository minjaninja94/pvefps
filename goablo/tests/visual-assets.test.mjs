...
test('four human hirelings isolate full silhouettes with safe padding',()=>{const def=SPRITE_MANIFEST.hirelingSummons,header=pngHeader(def.image);assert.deepEqual([header.width,header.height,header.colorType],[1536,768,6]);assert.deepEqual([def.columns,def.rows,def.chromaKey],[8,4,false]);assert.equal(def.isolateComponents,true);assert.ok(def.componentPadding>=20);assert.ok(def.componentAlphaThreshold>=24);assert.deepEqual(['hire0','hire1','hire2','hire3'].map(kind=>hirelingProfile(kind).row),[0,1,2,3]);assert.deepEqual(['hire0','hire1','hire2','hire3'].map(kind=>hirelingProfile(kind).name),['갈고리창 고아','문짝방패 고아','못박이 고아','두칼 고아']);});
